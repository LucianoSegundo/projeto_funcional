  ...
  versãoaberta <-  function (link_capitulo, capitulo_inicial, capitulo_final) {
    library(rvest)
    
    link_base <- link_capitulo
    {
    ...
    for(i in capitulo_inicial:capitulo_final){ 
    ...
    print(paste0("Todos os capitulo de ", titulo_daobra[4] , " foram baixado"))
  }
  
versãofechada <-  function (link_capitulo) {
    library(rvest)
  link_base <- link_capitulo
  {
    link_coleta <- paste0(link_base,'01')
